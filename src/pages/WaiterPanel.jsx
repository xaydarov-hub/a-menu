...
// Barcha funksiyalar to'liq ishlaydi, responsive, premium UI
// Telegram bot integratsiyasi, to'liq xatolik boshqaruvi
...
// ════════════════════════════════════════════════════════════════════════════════
// SCREEN: BILL - TO'LIQ HISOB (220 000 + 12% SERVIS HAQI)
// ════════════════════════════════════════════════════════════════════════════════
function ScreenBill({ table, settings, onBack, onPayment }) {
  const [vipSelected, setVipSelected] = useState(false);
  const [showConfirm, setShowConfirm] = useState(false);

  // BISETKA_INFO dan ma'lumotlarni olish
  const VIP_PRICE = parseInt(settings?.vip?.price || BISETKA_INFO.price || 220000);
  const SERVICE_FEE_PERCENT = parseFloat(settings?.vip?.serviceFee || BISETKA_INFO.serviceFeePercent || 12);
  const SERVICE_FEE = SERVICE_FEE_PERCENT / 100;
  ...
  // HISOB KALKULYATSIYASI
  const subtotal = grouped.reduce((s, i) => s + i.price * i.qty, 0);
  const serviceFeeAmount = Math.round(subtotal * SERVICE_FEE);
  const vipCost = vipSelected ? VIP_PRICE : 0;
  const total = subtotal + serviceFeeAmount + vipCost;
  ...
  return (
    ...
    >
        ...
        >
          ...
          <div style={{ padding: "14px 20px", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              <span style={{ color: "#7fa86b", fontSize: 14 }}>Mahsulotlar summasi</span>
              <span style={{ color: "#e8f5e0", fontSize: 14, fontWeight: 600 }}>{fmt(subtotal)}</span>
            </div>

            {/* Servis haqi - 12% */}
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              <span style={{ color: "#7fa86b", fontSize: 14 }}>
                Servis haqi ({SERVICE_FEE_PERCENT}%)
              </span>
              <span style={{ color: "#86B054", fontSize: 14, fontWeight: 600 }}>
                {fmt(serviceFeeAmount)}
              </span>
            </div>
            ...
            {vipSelected && (
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ color: "#D4AF37", fontSize: 14 }}>🥂 VIP xizmat (Bisetka)</span>
      ...
      {showConfirm && (
        <Confirm
          msg={`Jami: ${fmt(total)}\n(${fmt(subtotal)} + ${SERVICE_FEE_PERCENT}% servis = ${fmt(serviceFeeAmount)}${vipSelected ? ` + Bisetka ${fmt(VIP_PRICE)}` : ""})\n\nTo'lovni tasdiqlaysizmi?`}
          onYes={() => {
            setShowConfirm(false);
            onPayment({ total, vip: vipSelected, subtotal, serviceFee: serviceFeeAmount, serviceFeePercent: SERVICE_FEE_PERCENT });
          }}
          ...
          >
            ...
            <div style={{ padding: "20px 22px" }}>
              {[
                ...
                [`Servis haqi (${billInfo.serviceFeePercent || 12}%)`, fmt(billInfo.serviceFee)],
    ...
    >
      ...
      <div style={{ padding: "22px 20px" }}>
        ...
        >
          ...
          <div style={{ color: "#4a7a40", fontSize: 11, marginTop: 4 }}>
            Servis haqi: {billInfo.serviceFeePercent || 12}%
          </div>
        </div>
...
// ════════════════════════════════════════════════════════════════════════════════
// SCREEN: RATING - OFITSIANT PROFILI UCHUN REYTING
// ════════════════════════════════════════════════════════════════════════════════
function ScreenRating({ table, onDone, toast }) {
  ...
  const StarRow = ({ value, onChange, label, emoji }) => (
        ...
        >
          ...
          <div style={{ marginBottom: 22 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginBottom: 10 }}>
              {["Ajoyib!", "Tez xizmat", "Mazali ovqat", "Yana kelamiz", "Sog'lom muhit", "Super ofitsiant!", "Barakalla"].map((hint) => (
                <button
                  key={hint}
...
export default function WaiterPanel() {
  ...
  const handleUpdateShot = async (updatedShot) => {
    ...
    try {
      ...
      // 📤 TELEGRAMGA YUBORISH
      const orderTotal = updatedShot.items.reduce((s, i) => s + i.price * i.qty, 0);
      const orderMessage = `
    ...
    if (tbl) {
      try {
        ...
        await dbSet(`orders/${orderId}`, {
          ...
          serviceFeePercent: info.serviceFeePercent || 12,
          vip: info.vip,
          date: today(),
...
💁 Servis haqi (${info.serviceFeePercent || 12}%): ${fmt(info.serviceFee)}
${info.vip ? `🥂 VIP xizmat (Bisetka): ${fmt(info.total - info.subtotal - info.serviceFee)}` : ""}
─────────────────
