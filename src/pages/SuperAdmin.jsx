// SuperAdmin.jsx - TO'LIQ TUGALLANGAN VERSIYA
// Barcha bug'lar tuzatilgan, SeedMenuData qo'shilgan, Telegram bot to'liq ishlaydi
...
import { seedMenuData } from "../seedMenuData"; // ⭐ Yangi - menyu ma'lumotlarini yuklash
...
const IC = {
  ...
  scale:     ["M12 3v18","M5 7h14","M3 7l3 7a3 3 0 006 0L9 7","M15 7l3 7a3 3 0 006 0l-3-7"],
  clean:     ["M3 3l18 18","M21 3L3 21","M9 9l6 6"],
  bot:       ["M12 2v4","M5 10h14a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2z","M9 16v0","M15 16v0"],
...
// ─── CONFIRM ─────────────────────────────────────────────────────────────────
function Confirm({ msg, onYes, onNo, busy }) {
  return (
    <Modal title="Tasdiqlash" onClose={onNo}>
      <p style={{ color:"#e8f5e0", marginBottom:24, lineHeight:1.7, fontSize:14, whiteSpace:"pre-line" }}>{msg}</p>
      <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
        <button onClick={onYes} disabled={busy} style={{ ...BTN.danger, flex:1, minWidth:120, opacity:busy?0.6:1 }}>{busy?"Bajarilmoqda...":"Ha, tasdiqlash"}</button>
  ...
  return (
    <div>
      ...
      <div style={{ background:"rgba(39,174,96,0.08)", border:"1px solid rgba(39,174,96,0.25)", borderRadius:10, padding:"10px 16px", marginBottom:14, fontSize:12, color:"#2ecc71", display:"flex", alignItems:"flex-start", gap:8 }}>
        <span>✓</span>
        <span>Stollar <code style={{ background:"rgba(0,0,0,0.2)", borderRadius:4, padding:"1px 6px" }}>tableData.js</code> faylida belgilangan (zona, soni, sig'imi), holati esa Firebase <code style={{ background:"rgba(0,0,0,0.2)", borderRadius:4, padding:"1px 6px" }}>tables/</code> orqali real-time keladi.</span>
      </div>
...
// ─── WAITER MANAGER ──────────────────────────────────────────────────────────
function WaiterManager({ waiters, toast, isSuperAdmin, currentUser }) {
  ...
  const [pointsModalId, setPointsModalId] = useState(null);
  const [pAmount, setPAmount] = useState("");
  const [pType,   setPType]   = useState("penalty");
  ...
  const save = async () => {
    ...
    try {
      if (modal==="add") {
        ...
        const id = "wtr_"+uid();
        await dbSet(`waiters/${id}`, { ...form, points:100, orders:0, createdAt:today(), lastLogin:"—" });
        toast("Ofitsiant qo'shildi ✓ (Boshlang'ich ball: 100)","success");
      } else {
        const { id, ...data } = form;
  ...
  const ballColor = (p) => p>=80?"#2ecc71":p>=40?"#f39c12":p>=0?"#e67e22":"#e74c3c";

  const openPoints = (id) => {
    if (!isSuperAdmin) return;
    setPointsModalId(id); setPAmount(""); setPType("penalty"); setPReason("");
  };

  const applyAdjustment = async () => {
    const w = waitersObj[pointsModalId];
    if (!w) return;
    const amt = parseInt(pAmount);
    if (isNaN(amt) || amt<=0) { toast("Ball miqdorini to'g'ri kiriting","error"); return; }
    if (!pReason.trim()) { toast("Sabab yozish shart — halollik uchun bu muhim","error"); return; }
    setPSaving(true);
    const signed   = pType==="penalty" ? -amt : amt;
    const newTotal = (w.points!=null?w.points:100) + signed;
    const histId   = "h_"+uid();
    try {
      await dbUpdate(`waiters/${pointsModalId}`, {
        points: newTotal,
        [`pointHistory/${histId}`]: {
          amount: signed,
          reason: pReason.trim(),
      ...
      toast(pType==="penalty" ? `−${amt} ball yozildi` : `+${amt} ball qo'shildi`, pType==="penalty"?"warn":"success");
      setPAmount(""); setPReason("");
    } catch (e) { toast("Xatolik: "+e.message,"error"); }
    setPSaving(false);
  };
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))", gap:12 }}>
        {list.map(([id, w]) => {
          const pts = w.points!=null ? w.points : 100;
          const pc  = ballColor(pts);
          return (
            <div key={id} style={{ ...CARD, padding:"16px 18px" }}>
              ...
              <div onClick={()=>openPoints(id)}
                style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:`${pc}12`, border:`1px solid ${pc}40`, borderRadius:8, padding:"8px 12px", marginBottom:10, cursor:isSuperAdmin?"pointer":"default" }}>
                <div>
                  <div style={{ color:"#4a7a40", fontSize:9, letterSpacing:1 }}>BALL</div>
                  <div style={{ color:pc, fontSize:18, fontWeight:700, fontFamily:"Cinzel,serif" }}>{pts}</div>
                </div>
                {isSuperAdmin && (
                  <span style={{ color:pc, fontSize:11, border:`1px solid ${pc}50`, borderRadius:6, padding:"4px 10px" }}>⚖ Boshqarish</span>
                )}
              </div>
        ...
        <Modal title={modal==="add"?"Ofitsiant qo'shish":"Ofitsiant tahrirlash"} onClose={()=>setModal(null)}>
          ...
          {modal==="add" && (
            <div style={{ background:"rgba(212,175,55,0.1)", border:"1px solid rgba(212,175,55,0.3)", borderRadius:8, padding:"10px 14px", marginBottom:14, fontSize:12, color:"#D4AF37" }}>
              💡 Login: <b>{form.username}</b> · Parol: <b>{form.password}</b> · Boshlang'ich ball: <b>100</b>
            </div>
          )}
          <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
            <button style={{ ...BTN.gold, flex:1, minWidth:120, opacity:saving?0.7:1 }} onClick={save} disabled={saving}>
      ...
      {pointsModalId && (() => {
        const w = waitersObj[pointsModalId];
        if (!w) return null;
        const history = Object.entries(w.pointHistory||{}).sort((a,b)=> (a[0]<b[0]?1:-1));
        const pts = w.points!=null ? w.points : 100;
        return (
          <Modal title={`${w.name} — Ball boshqarish`} onClose={()=>setPointsModalId(null)} wide>
            <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:18 }}>
              <div style={{ color:"#4a7a40", fontSize:11 }}>Joriy ball</div>
              <div style={{ color:ballColor(pts), fontSize:26, fontWeight:700, fontFamily:"Cinzel,serif" }}>{pts}</div>
            </div>

            <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(134,176,84,0.15)", borderRadius:10, padding:16, marginBottom:18 }}>
              <div style={{ color:"#D4AF37", fontSize:11, letterSpacing:1, marginBottom:12 }}>YANGI BALL O'ZGARISHI</div>
              <div style={{ display:"flex", gap:8, marginBottom:12 }}>
                <button onClick={()=>setPType("penalty")} style={{ flex:1, padding:"9px", borderRadius:8, cursor:"pointer", background:pType==="penalty"?"rgba(192,57,43,0.25)":"rgba(255,255,255,0.04)", border:pType==="penalty"?"1px solid rgba(192,57,43,0.6)":"1px solid rgba(255,255,255,0.1)", color:pType==="penalty"?"#e74c3c":"#7fa86b", fontSize:13 }}>− Ball ayirish</button>
                <button onClick={()=>setPType("bonus")} style={{ flex:1, padding:"9px", borderRadius:8, cursor:"pointer", background:pType==="bonus"?"rgba(39,174,96,0.25)":"rgba(255,255,255,0.04)", border:pType==="bonus"?"1px solid rgba(39,174,96,0.6)":"1px solid rgba(255,255,255,0.1)", color:pType==="bonus"?"#2ecc71":"#7fa86b", fontSize:13 }}>+ Ball qo'shish</button>
              </div>
              <Field label="Miqdor" value={pAmount} onChange={setPAmount} type="number" placeholder="10" required/>
              <Field label="Sabab (majburiy — halollik uchun)" value={pReason} onChange={setPReason} type="textarea" placeholder="Masalan: mijozga qo'pol munosabat bildirgan, kechikkan, shikoyat tushgan..." required/>
              <button style={{ ...BTN.gold, width:"100%", opacity:pSaving?0.7:1 }} onClick={applyAdjustment} disabled={pSaving}>
                {pSaving?"Saqlanmoqda...":pType==="penalty"?"Ballni ayirish":"Ball qo'shish"}
              </button>
            </div>

            <div style={{ color:"#D4AF37", fontSize:11, letterSpacing:1, marginBottom:10 }}>BALL TARIXI</div>
            {history.length===0 ? (
              <div style={{ color:"#3d5c38", textAlign:"center", padding:24, fontSize:13 }}>Hali o'zgarish bo'lmagan</div>
            ) : (
              <div style={{ maxHeight:240, overflowY:"auto", display:"flex", flexDirection:"column", gap:6 }}>
                {history.map(([hid,h])=>(
                  <div key={hid} style={{ background:"rgba(255,255,255,0.03)", borderRadius:8, padding:"10px 14px" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
                      <span style={{ color:h.amount<0?"#e74c3c":"#2ecc71", fontWeight:700, fontSize:14 }}>{h.amount>0?"+":""}{h.amount} ball</span>
                      <span style={{ color:"#4a7a40", fontSize:11 }}>{h.date}</span>
                    </div>
                    <div style={{ color:"#e8f5e0", fontSize:12, marginBottom:2 }}>{h.reason}</div>
                    <div style={{ color:"#7fa86b", fontSize:10 }}>— {h.by}</div>
                  </div>
                ))}
              </div>
            )}
...
function MenuManager({ menu, categories, toast }) {
  ...
  const blank = { name:"", category:"", price:"", description:"", status:"active", image:"", emoji:"🍽️", categoryType:"food" };
  const [form, setForm] = useState(blank);
  const f = (k) => (v) => setForm((p) => ({ ...p, [k]: v }));
          ...
          {waiterStats.length===0 ? (
            ...
            <div key={w.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", borderBottom:i<4?"1px solid rgba(255,255,255,0.05)":"none" }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ color:"#4a7a40", fontSize:11 }}>{w.wCount} ta buyurtma · Ball: <span style={{ color:(w.points!=null?w.points:100)>=40?"#86B054":"#e74c3c" }}>{w.points!=null?w.points:100}</span></div>
              </div>
...
// ─── SETTINGS ─────────────────────────────────────────────────────────────────
function Settings({ toast, isSuperAdmin, currentUser, orders, waiters, tables }) {
  ...
  const saveTelegram = async () => {
    try { await dbUpdate("settings/telegram", telegram); toast("Telegram sozlamalari saqlandi ✓","success"); }
    catch { toast("Xatolik","error"); }
  };
  ...
  const buildStatsReport = () => {
    ...
    const todayList    = oList.filter((o)=>o.date===today());
    const todayTotal   = todayList.reduce((s,o)=>s+(o.total||0),0);
    const totalAll      = oList.reduce((s,o)=>s+(o.total||0),0);
    const waitersObj    = waiters||{};
    const waiterLines = Object.entries(waitersObj).map(([id,w])=>{
      const wOrds = oList.filter((o)=>o.waiterId===id);
      const rev   = wOrds.reduce((s,o)=>s+(o.total||0),0);
      return `• ${w.name}: ${wOrds.length} ta buyurtma, ${fmt(rev)}, Ball: ${w.points!=null?w.points:100}`;
    }).join("\n");

    return [
      `📊 ${restaurant.name||"AMAZONIA"} — Statistika hisoboti`,
      `🗓 ${nowStr()}`,
      ``,
      `💰 Bugungi tushum: ${fmt(todayTotal)} (${todayList.length} ta buyurtma)`,
      `💰 Jami tushum: ${fmt(totalAll)} (${oList.length} ta buyurtma)`,
      ``,
      `👨‍🍳 Ofitsiantlar:`,
      waiterLines || "— Ma'lumot yo'q",
      ``,
      `⚠️ Bu hisobot tozalashdan OLDIN yuborildi. Tozalashdan keyin buyurtmalar va faol stol hisoblari o'chadi, lekin ofitsiantlar, ularning ballari, kategoriya va menyular saqlanib qoladi.`,
    ].join("\n");
  };
  ...
  const doClearStatistics = async () => {
    setClearing(true);
    try {
      const report = buildStatsReport();
      const sent   = await sendToTelegram(report);

      await dbRemove("orders");

      const tablesArr = Array.isArray(tables) ? tables : [];
      await Promise.all(tablesArr.map((t) => dbUpdate(`tables/${t.id}`, { status:"free", shots:[] })));

      const waitersObj = waiters||{};
      await Promise.all(Object.keys(waitersObj).map((id) => dbUpdate(`waiters/${id}`, { orders:0 })));

      toast(
        sent
          ? "Statistika tozalandi va hisobot Telegramga yuborildi ✓"
          : "Statistika tozalandi ✓ (Telegram bot sozlanmagan yoki yuborilmadi)",
        "success"
      );
    } catch (e) {
  ...
  const handleSeedMenu = async () => {
    if (!window.confirm("⚠️ Barcha 87 ta mahsulot va 12 ta kategoriya Firebase ga yuklanadi.\n\nFaqat bir marta bosing! Qayta bosilsa dublikatlar yuzaga keladi.")) return;
    
    setSeedLoading(true);
    try {
      const result = await seedMenuData();
      if (result.success) {
        toast(`✅ ${result.items} ta mahsulot va ${result.categories} ta kategoriya yuklandi!`, "success");
        
        // 📤 Telegramga xabar yuborish
        const msg = `🌿 AMAZONIA MENYU YUKLANDI!\n\n📊 ${result.items} ta mahsulot\n📁 ${result.categories} ta kategoriya\n🕐 ${nowStr()}\n\n✅ Barcha ma'lumotlar Firebase ga muvaffaqiyatli yuklandi!`;
        await sendTelegramToAll(msg);
      } else {
        toast(`❌ Xatolik: ${result.error}`, "error");
      }
    } catch (err) {
  ...
  return (
    <div style={{ display:"grid", gap:16 }}>
      ...
      {isSuperAdmin && (
        <div style={CARD}>
          <div style={CARD_HDR}><span style={CARD_TTL}>🤖 Telegram bot (statistika yuborish)</span></div>
          <div style={{ color:"#7fa86b", fontSize:12, marginBottom:14, lineHeight:1.7 }}>
            "Statistikani tozalash" bosilganda hisobot avtomatik shu botga yuboriladi. Bot tokenini @BotFather orqali, Chat ID ni @userinfobot yoki botni guruhga qo'shib olishingiz mumkin.
          </div>
          <div className="field-2col" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0 20px" }}>
            <Field label="Bot Token" value={telegram.botToken} onChange={(v)=>setTelegram((p)=>({...p,botToken:v}))} placeholder="123456789:ABC-DEF1234ghIkl..."/>
            <Field label="Chat ID"   value={telegram.chatId}   onChange={(v)=>setTelegram((p)=>({...p,chatId:v}))}   placeholder="-1001234567890"/>
          </div>
          <button style={BTN.gold} onClick={saveTelegram}>Saqlash</button>
        </div>
      )}

      {/* ⭐🌟🌟 MENYUNI YUKLASH 🌟🌟🌟 */}
      {isSuperAdmin && (
        <div style={{ ...CARD, border:"1px solid rgba(212,175,55,0.3)", background:"rgba(212,175,55,0.05)" }}>
          <div style={CARD_HDR}>
            <span style={{ ...CARD_TTL, color:"#D4AF37" }}>🌿 Menyuni Firebase ga yuklash</span>
            <span style={{ color:"#4a7a40", fontSize:10 }}>⭐ Yangi</span>
          </div>
          <div style={{ color:"#7fa86b", fontSize:12, marginBottom:14, lineHeight:1.8 }}>
            Bu tugma orqali <b style={{ color:"#D4AF37" }}>87 ta mahsulot</b> va <b style={{ color:"#D4AF37" }}>12 ta kategoriya</b> Firebase ga yuklanadi.
            <br/>
            ⚠️ <b style={{ color:"#e74c3c" }}>DIQQAT:</b> Faqat <b>BIR MARTA</b> bosing! Qayta bosilsa dublikat ma'lumotlar yuzaga keladi.
            <br/>
            ✅ Yuklangandan so'ng menyu sahifasida barcha taomlar ko'rinadi.
          </div>
          <button
          ...
          >
            {seedLoading ? (
              ...
              <>
                <Icon d={IC.upload} size={18} color="#0a1f0d" />
                🌿 Menyuni yuklash (87 ta mahsulot)
              </>
            )}
          </button>
        </div>
      ...
      {isSuperAdmin && (
        <div style={{ ...CARD, border:"1px solid rgba(192,57,43,0.3)" }}>
          <div style={CARD_HDR}><span style={{ ...CARD_TTL, color:"#e74c3c" }}>🧹 Statistikani tozalash</span></div>
          <div style={{ color:"#7fa86b", fontSize:12, marginBottom:14, lineHeight:1.8 }}>
            Bu amal faqat <b style={{ color:"#e8f5e0" }}>buyurtmalar va pul bilan bog'liq</b> ma'lumotlarni (statistika, faol stol hisoblari) butunlay tozalaydi.
            <br/>✓ Ofitsiantlar, ularning <b style={{ color:"#2ecc71" }}>ballari va ball tarixi</b>, kategoriyalar va menyu — <b style={{ color:"#2ecc71" }}>saqlanib qoladi</b>.
            <br/>Tozalashdan oldin to'liq hisobot avtomatik Telegram botga yuboriladi.
          </div>
          <button style={BTN.danger} onClick={()=>setClearConfirm(true)}>🧹 Statistikani tozalash</button>
        </div>
      )}
      ...
      {clearConfirm && (
        <Confirm
          busy={clearing}
          msg={"Barcha buyurtmalar va faol stol hisoblari BUTUNLAY o'chiriladi.\nOfitsiantlar, ularning ballari, kategoriya va menyular saqlanadi.\nHisobot avval Telegramga yuboriladi.\n\nDavom etasizmi?"}
          onYes={doClearStatistics}
          onNo={()=>!clearing && setClearConfirm(false)}
        />
      )}
    </div>
  );
...
export default function SuperAdmin() {
  ...
  const pages = {
    ...
    waiters:    <WaiterManager waiters={waiters} toast={toast} isSuperAdmin={isSuperAdmin} currentUser={user}/>,
    ...
    settings:   <Settings     toast={toast} isSuperAdmin={isSuperAdmin} currentUser={user} orders={orders} waiters={waiters} tables={tables}/>,
  };
