import { useState, useEffect, useCallback, useRef } from "react";
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // WAITER MANAGER
  // YANGI: Ofitsiantlar reytingi endi 5 yulduzli emas — BALL tizimi.
  //   • Har bir ofitsiant boshlang'ich 100 ball bilan boshlaydi (waiters/{id}/points).
  //   • Faqat Super Admin ball qo'shishi yoki AYIRISHI mumkin — har doim SABAB
  //     yozish majburiy (halollik uchun — mijoz ko'rmagan kamchiliklar shu yerda
  //     qayd etiladi).
  //   • Har bir o'zgarish waiters/{id}/pointHistory ichida vaqt, sabab va
  //     kim bajarganligi bilan saqlanadi — to'liq tarix ko'rinadi.
  //   • Ball doimiy saqlanadi — "Statistikani tozalash" bosilganda bu ma'lumot
  //     o'chmaydi (faqat buyurtma/pul tarixi tozalanadi).
  // ════════════════════════════════════════════════════════════════════════════
  function WaiterManager({ waiters, toast, isSuperAdmin, currentUser }) {
    ...
    const [pType,   setPType]   = useState("penalty"); // penalty | bonus
    ...
    const openPoints = (id) => {
      if (!isSuperAdmin) return; // faqat Super Admin ball tarixi/boshqaruvini ochadi
    ...
    return (
      <div>
        ...
        {/* BALL BOSHQARISH / TARIX MODAL — faqat Super Admin uchun ochiladi */}
        {pointsModalId && (() => {
          const w = waitersObj[pointsModalId];
          if (!w) return null;
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // SETTINGS
  // YANGI: 🤖 Telegram bot sozlamalari + 🧹 Statistikani tozalash.
  //   "Statistikani tozalash" bosilganda:
  //     1) Hozirgi to'liq hisobot (tushum, buyurtmalar, ofitsiantlar) avval
  //        Telegram botga yuboriladi (agar bot sozlangan bo'lsa).
  //     2) "orders/" tugun butunlay o'chiriladi, faol stol hisoblari (shots)
  //        tozalanadi — FAQAT pul/buyurtma bilan bog'liq narsalar.
  //     3) Ofitsiantlar, ularning BALLI va ball tarixi, kategoriyalar va
  //        menyu — TEGILMAYDI, butunlay saqlanib qoladi.
  // ════════════════════════════════════════════════════════════════════════════
  function Settings({ toast, isSuperAdmin, currentUser, orders, waiters, tables }) {
    ...
    const doClearStatistics = async () => {
      ...
      try {
        ...
        // 1) Buyurtmalarni butunlay tozalash (pul/zakaz bilan bog'liq)
        await dbRemove("orders");

        // 2) Faol stol hisoblarini (shots) bo'sh holatga keltirish
        const tablesArr = Array.isArray(tables) ? tables : [];
        await Promise.all(tablesArr.map((t) => dbUpdate(`tables/${t.id}`, { status:"free", shots:[] })));

        // 3) Ofitsiantlarning buyurtma sonini nollash — BALL va ball
        //    tarixi (pointHistory) hech qachon TEGILMAYDI
        const waitersObj = waiters||{};
